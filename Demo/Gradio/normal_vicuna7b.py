...
def multi_modal_pipeline(image_file, prompt):
...
def gradio_interface(image_file, text_query, translate):
    translated_text = text_query
    if translate:
        translated_text = translate_text(text_query)
    output = multi_modal_pipeline(image_file, text_query)
...
iface = gr.Interface(
    ...
    inputs=[
        ...
        gr.components.Textbox(lines=2, placeholder="Enter text query here...", label="Text Query"),
        gr.components.Checkbox(label="Translate to English")
    ],
    outputs=[
        gr.Textbox(label="Translated Text/Original Text"),
        gr.Textbox(label="Output")
    ],
    title="Multimodal Summ&Translation Demo",
    description="Upload an image and enter a text query. Please tick 'Translate' in case of languages other than english and hindi."
)
